...
const DETECTOR_SHA = '0b5d50b64ca7442b1259a41cc7d8f8a0ef398c1c';
...
const CUSTOM_LIST_PATH = join(
  REPO_ROOT,
  'actions',
  'security-scan',
  'compromised-packages-custom.txt'
);
...
function fetchUpstreamList(): string {
  ...
  console.log(
    `${c.dim}   Fetching upstream compromised-packages.txt (SHA: ${DETECTOR_SHA.slice(0, 12)}...)${c.reset}`
  );
  ...
  try {
    ...
    try {
      console.log(
        `${c.yellow}   Warning: Could not fetch upstream list, using stale cache${c.reset}`
      );
      return readFileSync(CACHE_FILE, 'utf-8');
    } catch {
      console.log(
        `${c.red}   Error: Could not fetch upstream list and no cache available${c.reset}`
      );
      throw new Error(
        'Could not obtain compromised package database: upstream fetch failed and no cache available'
      );
    }
  }
}
  ...
  for (const rawLine of lines) {
    ...
    if (!map.has(key)) {
      map.set(key, {
        title:
          currentTitle ||
          (source === 'upstream' ? 'Shai-Hulud Supply Chain Attack' : 'Custom Blocklist'),
        url: currentUrl || '',
      });
    }
  }
...
function main() {
  ...
  console.log(
    `${c.dim}Loaded ${compromisedMap.size} compromised packages (${upstreamCount} upstream + ${customCount} custom).${c.reset}`
  );
  console.log('');
  ...
  for (const lockfile of lockfiles) {
    ...
    if (findings.length === 0) {
      ...
      console.log(
        `   ${c.red}❌ ${findings.length} vulnerable package${findings.length > 1 ? 's' : ''}:${c.reset}`
      );
      console.log('');
  ...
  console.log(`${c.cyan}${'═'.repeat(78)}${c.reset}`);
  console.log(
    `${c.dim}Scanned:    ${lockfiles.length} repos (${totalPackages.toLocaleString()} total packages)${c.reset}`
  );
  ...
  if (allFindings.size > 0) {
    ...
    console.log('');
    console.log(
      `${c.green}${c.bold}✅ All repos are clean — no compromised packages detected.${c.reset}`
    );
  }
